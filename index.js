...
app.use("/auth", authRoutes); // now app exists, this works
...
app.post("/newOrder", verifyToken, async (req, res) => {
  ...
  // 1. Log the raw order — unchanged, this part already works
  ...
  // 2. Reconcile Holdings — this is what was missing
  const existing = await HoldingsModel.findOne({ name });

  if (mode === "BUY") {
    if (existing) {
      const totalQty = existing.qty + quantity;
      const totalCost = existing.avg * existing.qty + orderPrice * quantity;
      existing.avg = totalCost / totalQty;
      existing.qty = totalQty;
      existing.price = orderPrice;
      await existing.save();
    } else {
  ...
  } else if (mode === "SELL" && existing) {
    existing.qty -= quantity;
    if (existing.qty <= 0) await HoldingsModel.deleteOne({ name });
